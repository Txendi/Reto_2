...
const apuntarseEvento = async (eventoId) => {
  try {
    const res = await fetch(`http://localhost/eventos.php?accion=signup&id=${eventoId}`, {
      method: 'POST',
    })
    ...
    if (data.ok) {
      ...
      alert(data.error || 'No se pudo apuntar')
    }
  } catch (e) {
...
<template>
  <section class="max-w-7xl mx-auto px-4 pt-8 pb-25">
    ...
    <div class="flex flex-col sm:flex-row">
      ...
      <input
        type="date"
        class="text-black mb-5 p-2 border-2 rounded-xl sm:mx-3"
    ...
    <div class="flex justify-center mt-7 gap-8">
      <button
        class="bg-yellow-200 px-5 py-2 cursor-pointer hover:bg-white"
    ...
    <Transition name="fade">
      ...
      >
        <div class="bg-white rounded-xl w-full max-w-lg p-6 relative">
          ...
          <img
            :src="`/img/events/${eventoActivo.imagen}`"
          ...
          <button
            v-if="eventoActivo.plazasLibres > 0"
            @click.stop="apuntarseEvento(eventoActivo.id)"
