...
const plazasDisponibles = ref(false) //
const fechaSeleccionada = ref('')

const cargarEventos = async () => {
  try {
    const response = await fetch(
      `http://localhost/bbdd.php?action=listaEventos&pagina=${paginaActual.value}&tipo=${tipoSeleccionado.value}&fecha=${fechaSeleccionada.value}&plazasLibres  =${plazasDisponibles.value}`,
      {
        method: 'POST',
        headers: {
...
<template>
  <section class="max-w-7xl mx-auto px-4 pt-8 pb-25">
    ...
    <div class="flex items-center gap-6 mb-5">
      <select v-model="tipoSeleccionado" class="text-black p-2 border-2 rounded-xl w-3xl">
      ...
      <label class="flex items-center gap-2 text-white">
        <input type="checkbox" v-model="plazasDisponibles" />
